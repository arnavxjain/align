...
class AnalysisService {
  ...
  static Future<void> runAnalysis({
    ...
    // Auth guard — never send data to external APIs for unauthenticated users.
    ...
    try {
      ...
      // Title is generated by Gemini after content is available — no early fetch.
      ...
      // Generate headline title from actual content.
      final generatedTitle = await GeminiService.generateTitle(
        url: url,
        isYouTube: isYouTube,
        geminiFileUri: geminiFileUri,
      );
      if (generatedTitle != null && generatedTitle.isNotEmpty) {
        cur['title'] = generatedTitle;
        _update(id, cur);
      }
  ...
  static Future<void> saveCreativeSpace(
    String alignmentId,
    List<Map<String, dynamic>> messages,
  ) async {
    final now = DateTime.now().toUtc().toIso8601String();
    final creativeSpace = <String, dynamic>{
    ...
    if (idx >= 0) {
      final updated = Map<String, dynamic>.from(list[idx]);
      updated['creative_space'] = creativeSpace;
      list[idx] = updated;
      alignmentsNotifier.value = list;
    }
    ...
    try {
      ...
      final si = current.indexWhere((e) => e['id'] == alignmentId);
      if (si >= 0) {
        current[si]['creative_space'] = creativeSpace;
        await client
            .from('user_profiles')
            .update({'saved_analyses': current})
            .eq('id', userId);
  ...
  static Future<void> _store(Map<String, dynamic> entry) async {
    ...
    // Enforce 500-entry cap — fetch current count before writing.
    try {
      final data = await client
          .from('user_profiles')
          .select('saved_analyses')
          .eq('id', userId)
          .single();
      final count = ((data['saved_analyses'] as List?) ?? []).length;
      if (count >= 500) {
        debugPrint('AnalysisService: 500-entry cap reached, not storing.');
        return;
      }
    } catch (_) {}
